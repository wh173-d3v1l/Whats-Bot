...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'meme ?(.*)', fromMe: true, desc: Lang.MEMES_DESC}, (async (message, match) => {   
	    ...
	    memeMaker({
            image: location,         
            outfile: 'toxic-meme.png',
            topText: topText,
            bottomText: bottomText,
        }, async function(err) {
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('toxic-meme.png'), MessageType.image, {filename: 'toxic-meme.png', mimetype: Mimetype.png, caption: 'ᴍᴀᴅᴇ ʙʏ ᴛᴏxɪᴄ ᴍᴡᴏʟ [@ʀɪᴢᴀᴅ__x96]'});
            await info.delete();    
        });
    }));
}
